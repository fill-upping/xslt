...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template name="documentStructure">
      ...
      <xsl:for-each select="$topSection/da:products/da:product">
      ...
      <xsl:variable name="altHeadline" select="translate($bottomSection/da:fields/da:field[@name='Alternate Headline Style']/text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = 'yes'"/>
   ...
   <xsl:template name="productItem">
      ...
      <xsl:if test="not($groupedPosition &gt; 1)">
         ...
         <xsl:element name="ProductBox">
            ...
            <!-- add test to see if drink is Energy (only in venti) and display Venti data -->
               ...
               <xsl:choose>
                  <xsl:when test="$product[@id='25245' or @id='25246'] or contains($product/da:name,'Melon Burst') or contains($product/da:name,'Tropical Citrus')">
                     <xsl:call-template name="priceLineText">
                        ...
                        <xsl:with-param name="servingSize" select="da:sellableItems/da:sellableItem/da:servingSize[@code='Venti']" />
                        <xsl:with-param name="product" select="$product" />
                     ...
                     <xsl:call-template name="caffeineData">
                        <xsl:with-param name="caffeine" select="$product/da:sellableItems/da:sellableItem/da:servingSize[@code='Venti']/da:nutrition[@displayName='Caffeine']" />
                     </xsl:call-template>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:call-template name="priceLineText">
