...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
      ...
      <xsl:element name="Root">
         ...
         <xsl:call-template name="documentStructure">
            <xsl:with-param name="section" select="//da:transformation/da:templateData/da:sections/da:section[@name='FoodDuo' or 'FoodTrio' or 'Quad' or 'Hero']" />
         </xsl:call-template>
   ...
   <xsl:template name="documentStructure">
      ...
      <xsl:for-each select="$section/da:products/da:product">
   ...
   <xsl:template name="productItem">
      ...
      <xsl:element name="ProductBox">
         ...
         <xsl:text>&#x2029;</xsl:text>
         <!-- populate PriceLine element with details from serving size Serving (for food) -->
         <xsl:element name="{$priceLineElementName}">
            <xsl:call-template name="priceLineText">
               <xsl:with-param name="excludeCal" select="$excludeCal"/>
               <xsl:with-param name="excludeProtein" select="$excludeProtein"/>
               <xsl:with-param name="servingSize" select="$product/da:sellableItems/da:sellableItem/da:servingSize[@code='Serving' or 'Whole']" />
            </xsl:call-template>
         </xsl:element>
      </xsl:element>
   ...
   <xsl:template name="priceLineText">
      ...
      <xsl:if test="not($excludeProtein) and $hasPricing">
         <xsl:text>&#8197;|&#8197;</xsl:text>
      </xsl:if>
      ...
      <xsl:if test="not($excludeCal) and not($hasPricing) and not($excludeProtein)">
         <xsl:text>&#8197;|&#8197;</xsl:text>
      </xsl:if>
      <xsl:if test="not($excludeCal)">
         <xsl:call-template name="formattedNutrition">
