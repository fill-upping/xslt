...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
      <xsl:element name="Root">
         ...
         <xsl:call-template name="documentStructure">
            ...
            <xsl:with-param name="bottomLineListing" select="//da:transformation/da:templateData/da:sections/da:section[@name='BottomLineListings']" />
   ...
   <xsl:template name="documentStructure">
      ...
      <xsl:param name="bottomLineListing" />
      ...
      <xsl:element name="HeadlineCenter">
         ...
         <xsl:if test="$insetListing/da:fields/da:field[@name='Subheading1']/text()">
            <xsl:text>&#x2029;</xsl:text>
            <xsl:element name="Subhead">
               <xsl:value-of select="$insetListing/da:fields/da:field[@name='Subheading1']/text()" />
            </xsl:element>
         </xsl:if>
      </xsl:element>
      <xsl:for-each select="$insetListing/da:products/da:product[position()&lt;=8]">
      ...
      <!-- <xsl:for-each select="$bottomLineListing/da:products/da:product">
   ...
   <xsl:template name="productItem">
      ...
      <xsl:if test="not($groupedPosition &gt; 1)">
         ...
         <xsl:variable name="headlineElementName">
            <xsl:choose>
               <xsl:when test="$market = 'US'">PriceLineRed</xsl:when>
               <xsl:otherwise>PriceLineCANRed</xsl:otherwise>
            </xsl:choose>
         </xsl:variable>
