...
<?xml-model href="https://www.w3.org/2007/schema-for-xslt20.xsd" type="application/xml" schematypens="http://www.w3.org/2001/XMLSchema"?>
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
      ...
      <xsl:element name="Root">
         ...
         <xsl:if test="$footerSection/da:fields/da:field[@name='Copyright']/text()">
   ...
   <xsl:template name="documentStructure">
      ...
      <xsl:param name="excludeCal" select="false()" />
      ...
      <!-- allow for max 4 products -->
      <xsl:for-each select="$section/da:products/da:product[position() &lt;= 4]">
      ...
      <xsl:element name="ProductBox">
         ...
         <xsl:element name="{$priceLineElementName}">
            <xsl:call-template name="priceLineText">
               ...
               <xsl:with-param name="servingSizeName" select="$product/da:sellableItems/da:sellableItem/da:servingSize" />
            </xsl:call-template>
         </xsl:element>
      </xsl:element>
   </xsl:template>
   ...
   <xsl:template name="priceLineText">
      ...
      <xsl:param name="servingSizeName" />
      <xsl:param name="excludeCal" />
      <xsl:param name="excludeProtein" />
      ...
      <xsl:if test="not($excludeProtein) and $hasPricing and not($excludeCal)">
         <xsl:text>&#8197;|&#8197;</xsl:text>
      </xsl:if>
      <xsl:if test="$servingSizeName[@code='Grande']">
         <xsl:text>Grande</xsl:text>
         <xsl:text>&#8197;</xsl:text>
      </xsl:if>
      <xsl:if test="$hasPricing">
         ...
         <xsl:if test="not($excludeCal) and not($excludeProtein)">
            <xsl:text>&#8197;|&#8197;</xsl:text>
         </xsl:if>
      </xsl:if>
      <xsl:if test="not($excludeCal) and $hasPricing and $excludeProtein">
         <xsl:text>&#8197;|&#8197;</xsl:text>
      </xsl:if>
