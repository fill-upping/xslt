...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
      ...
      <xsl:element name="Root">
         ...
         <xsl:call-template name="documentStructure">
            <xsl:with-param name="topSection" select="//da:transformation/da:templateData/da:sections/da:section[@name='CoreFood']" />
            <xsl:with-param name="bottomSection" select="//da:transformation/da:templateData/da:sections/da:section[@name='PromoInset']" />
         </xsl:call-template>
         <xsl:if test="$footerSection/da:fields/da:field[@name='Regulatory']/text()">
            <xsl:element name="Legal">
               <xsl:value-of select="$footerSection/da:fields/da:field[@name='Regulatory']/text()" />
            </xsl:element>
         </xsl:if>
   ...
   <xsl:template name="documentStructure">
      <xsl:param name="cakePopArray" />
      <xsl:param name="topSection" />
      <xsl:param name="bottomSection" />

      <xsl:element name="HeadlineLeft">
         <xsl:value-of select="$topSection/da:fields/da:field[@name='Headline']/text()" />
      </xsl:element>
      <xsl:for-each select="$topSection/da:products/da:product[position()&lt;=8]">
         <xsl:call-template name="productItem">
            ...
            <xsl:with-param name="productNameElement" select="'ProductName'" />
            <xsl:with-param name="priceLineElement" select="'PriceLine'" />
      ...
      <xsl:element name="HeadlineCenter">
         <xsl:element name="HeadlineCenter">
            <xsl:value-of select="$bottomSection/da:fields/da:field[@name='Headline']/text()" />
            <xsl:if test="$bottomSection/da:fields/da:field[@name='Subheading1']/text()">
               <xsl:text>&#x2029;</xsl:text>
               <xsl:element name="Subhead">
                  <xsl:value-of select="$bottomSection/da:fields/da:field[@name='Subheading1']/text()" />
               </xsl:element>
            </xsl:if>
         </xsl:element>
      </xsl:element>
      <xsl:for-each select="$bottomSection/da:products/da:product[position()&lt;=3]">
         <xsl:call-template name="productItem">
            <xsl:with-param name="product" select="." />
            <xsl:with-param name="groupedPosition" select="number(substring-before(./da:altDescriptor/text(),'/'))" />
            <xsl:with-param name="groupNumber" select="number(substring-after(./da:altDescriptor/text(),'/'))" />
            <xsl:with-param name="includeNewBug" select="true()" />
            <xsl:with-param name="productNameElement" select="'ProductNameRed'" />
            <xsl:with-param name="priceLineElement" select="'PriceLineRed'" />
   ...
   <xsl:template name="productItem">
      ...
      <xsl:param name="productNameElement" />
      <xsl:param name="priceLineElement" />
      ...
      <xsl:if test="not($groupedPosition &gt; 1)">
         ...
         <xsl:element name="ProductBox">
            <xsl:element name="{$productNameElement}">
               <xsl:value-of select="$product/da:name" />
            </xsl:element>
            <xsl:variable name="headlineElementName">
               <xsl:choose>
                  <xsl:when test="$market = 'US' and $priceLineElement = 'PriceLine'">PriceLine</xsl:when>
                  <xsl:when test="$market = 'US' and $priceLineElement = 'PriceLineRed'">PriceLineRed</xsl:when>
                  <xsl:when test="$market = 'CA' and $priceLineElement = 'PriceLine'">PriceLineCAN</xsl:when>
                  <xsl:when test="$market = 'CA' and $priceLineElement = 'PriceLineRed'">PriceLineCANRed</xsl:when>
               </xsl:choose>
            </xsl:variable>
            ...
            <xsl:element name="{$headlineElementName}">
               <!-- prepend "1 for" if merged cakepops -->
   ...
   <xsl:template name="priceLineText">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$showCalRange and number($groupedPosition)">
            <xsl:text>&#x2029;</xsl:text>
         </xsl:when>
         <xsl:otherwise>
            <xsl:text>&#x2002;|&#x2002;</xsl:text>
