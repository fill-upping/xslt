...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/">
      ...
      <xsl:element name="Root">
         ...
         <xsl:call-template name="documentStructure">
            <xsl:with-param name="topSection" select="//da:transformation/da:templateData/da:sections/da:section[@name='TopLineListings']" />
            <xsl:with-param name="bottomSection" select="//da:transformation/da:templateData/da:sections/da:section[@name='BottomLineListings']" />
         </xsl:call-template>
   ...
   <xsl:template name="documentStructure">
      ...
      <xsl:variable name="includeProteinBoolTop" select="translate($topSection/da:fields/da:field[@name='Exclude Protein']/text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')!='yes'"/>
      ...
      <xsl:for-each select="$topSection/da:products/da:product">
         <xsl:call-template name="productItem">
            ...
            <xsl:with-param name="includeProtein" select="$includeProteinBoolTop" />
         </xsl:call-template>
      </xsl:for-each>

      <xsl:variable name="includeProteinBoolBottom" select="translate($bottomSection/da:fields/da:field[@name='Exclude Protein']/text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')!='yes'"/>

      <xsl:element name="HeadlineLeft">
         <xsl:value-of select="$bottomSection/da:fields/da:field[@name='Headline']/text()" />
      </xsl:element>
      <xsl:for-each select="$bottomSection/da:products/da:product">
         <xsl:call-template name="productItem">
            <xsl:with-param name="product" select="." />
            <xsl:with-param name="groupedPosition" select="number(substring-before(./da:altDescriptor/text(),'/'))" />
            <xsl:with-param name="groupNumber" select="number(substring-after(./da:altDescriptor/text(),'/'))" />
            <xsl:with-param name="includeNewBug" select="true()" />
            <xsl:with-param name="includeProtein" select="$includeProteinBoolBottom" />
         </xsl:call-template>
      </xsl:for-each>
               ...
               <xsl:when test="$market = 'CA'">
                  <xsl:element name="ProteinCAN">
                     ...
                     <xsl:call-template name="priceLineText">
                        ...
                        <xsl:with-param name="maxCalRange" select="$product/da:altDescriptor/text()" />
                        <xsl:with-param name="product" select="$product" />
